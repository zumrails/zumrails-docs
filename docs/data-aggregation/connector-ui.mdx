...
After a new connection is made, if you are using the Connector UI, the data can be available to you in different ways, listed bellow:

### Access the data in Zūm Rails portal

For every new connection made for an specific card, a new user will be automatically created in Zūm Rails portal, you can search it under the User page by the user name of email. In this option, zero integration is required, but it's a bit difficult to match the user, since the name in the bank account might be different than the name you have your our file. 

### Access the data via javascript event listener

Every interaction with the UI triggers a javascript event listener that your host page can listen to. Each message has a step as described bellow.

To add the event listener on your website:
