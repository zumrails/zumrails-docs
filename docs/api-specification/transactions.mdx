...
InteracSecurityQuestion | string | yes | The question for the user to process the interac request. Required if InteracHasSecurityQuestionAndAnswer is true
InteracSecurityAnswer | string | yes | The answer for the user to process the interac request. Required if InteracHasSecurityQuestionAndAnswer is true
...
The main difference in Interac peer-to-peer is the fact that you have to inform two different users. The payload will be like:
...
In case you are doing an Accounts Payable - push - transction, it's important that you call the endpoint [Interac - Send Funds options](../api-specification/users#interac---send-funds-options) to determine the options available for the target user.
...
:::info
If your Zūm Rails account is configured to user Interac peer-to-peer, you will only be authorized to do Interac transactions. If you need to user other rails, a new Zūm Rails environment will be needed.
:::
