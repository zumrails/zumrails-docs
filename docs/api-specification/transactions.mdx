...
InteracSecurityQuestion | string [ 5 .. 40 ] characters ^[^\s]([a-zA-Z0-9àâäèéêëîïôœùûüÿçÀÂÄÈÉÊËÎÏÔŒÙÛÜŸÇ \-\.\?\!,#]+)[^\s]$ | yes | The question for the user to process the interac request. Required if InteracHasSecurityQuestionAndAnswer is true
InteracSecurityAnswer | string [ 3 .. 25 ] characters [a-zA-Z0-9àâäèéêëîïôœùûüÿçÀ ÄÈÉÊËÎÏÔŒÙÛÜŸÇ]+$... | yes | The answer for the user to process the interac request. Required if InteracHasSecurityQuestionAndAnswer is true
