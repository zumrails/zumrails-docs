...
2. **Create a token** which has to be informed in the SDK. Use the endpoint `{{env}}/api/connect/createtokensdk` to generate the token. The token is valid for **15 minutes**, and it will be automatically expired after a user uses it.
...
<Tabs
  defaultValue="tab1"
  values={[
    {label: 'Add a payment profile', value: 'tab1'},
  ]}>
  <TabItem value="tab1">
